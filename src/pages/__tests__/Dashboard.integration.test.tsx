...
describe('Dashboard Integration Tests', () => {
  ...
  // Use existing LivingLab (for example, test account already has one)
  ...
  beforeAll(async () => {
    ...
    // Create our experiments via API
  ...
  test('FETCH LIVINGLABS AND EXPERIMENTS ON MOUNT', async () => {
    ...
    // Verify that every experiment (from the API) is rendered (by unique row test ID)
    for (const experiment of myExperiments) {
      expect(screen.getByTestId(`experiment-row-${experiment.ID}`)).toBeInTheDocument();
    }

    // Open the LivingLab dropdown and narrow the query to its content
  ...
  test('RENDER LOADING STATE', async () => {
    ...
    // Initially the loading container is visible
  ...
  test('FILTERING UPDATES SORTED LIST', async () => {
    const myExperiments = await getExperiments();
    const ourExperiments = myExperiments.filter(exp => exp.name.includes(uniqueId));
    ...
    // Wait for our experiments to render
    ...
    // Click the "Responses" header to sort the table
